	...
	float3 fReflection	: TEXCOORD3) : COLOR0
{
	...
	//float4 Color = tex2D(g_samScene, TexCoord) * float4(color + specular, 1.0f); // 버그 수정 전
	float4 Color = tex2D(g_samScene, TexCoord) + float4(color + specular, 1.0f) / 2.0f; //  버그 수정 후
	return Color;
}
...
VS_OUTPUT VertSkinning( VS_INPUT Input, uniform int nNumBones )
{
	...
	//Output.Diffuse = ceil(Output.Diffuse * 5) / 5.0f; //카툰 렌더링 연산
